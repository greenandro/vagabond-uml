...
exports.createParser = function(args) {
  function parse(content, files) {
      try {
        var parsed = pegParser.parse(content);
        return parsed;            
      } catch (e) {
  ...
  if (args.file) { // standard mode
    if (endsWith(args.file, '.vg') || endsWith(args.file, '.vag')) {
      var content = readJDLFile(args.file);
      var parsed = parse(content, args.file);
      return new Editors.Parsers.dsl(parsed, types);
    } else {
      return getXMIFileParser(args.file, types);
    }
  } else if (args.files) {
    if (areFilesVgbFiles(args.files)) {
      var content = aggregateFiles(args.files);
      var parsed = parse(content, args.files);
      return new Editors.Parsers.dsl(parsed, types);
    } else {
      throw new buildException(
          exceptions.IllegalArgument,
...
function areFilesVgbFiles(files) {
  return files.every(function(file) {
    return endsWith(file, '.vg') || endsWith(file, '.vag');
  });
}

...
if (process.argv.length < 3) {
  throw new ArgumentException(
    ...
    + "Use the command 'vagabond-uml -help' to see the available commands.");
}
...
if (!options.db && !fs.existsSync('./.yo-rc.json')) {
  throw new ArgumentException(
    ...
    + "Use the command 'vagabond-uml -help' to see the available options."
  );
} else {
...
function initParserFactoryArgs() {
  ...
  if (options.files.length === 1) {
    ...
    console.error('At least one file to parsed must be passed.');
    process.exit(1);
  }
